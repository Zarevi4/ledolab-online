...
export default function Process() {
  return (
    <section className="py-28 sm:py-36 px-6 lg:px-8">
      <div className="max-w-[1200px] mx-auto">
        ...
        >
          ...
          <p className="mt-4 text-[16px] text-ink-light leading-relaxed">
            6 простых шагов от первого звонка до масштабирования вашего бизнеса.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {processSteps.map((step, i) => (
            <motion.div
              key={step.number}
              {...fade(i)}
              className="group relative p-6 rounded-[20px] border border-border bg-surface hover:shadow-[0_12px_40px_rgba(0,0,0,0.05)] hover:border-accent/20 transition-all duration-500"
            >
              <div className="w-10 h-10 rounded-xl gradient-bg text-white flex items-center justify-center mb-4 text-[14px] font-bold">
                {step.number}
              </div>
