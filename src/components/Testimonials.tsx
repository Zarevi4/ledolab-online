...
const testimonials = [
  { name: "Алексей К.", role: "eCommerce, Германия", text: "За 3 месяца работы с LedoLab мы увеличили количество лидов в 4 раза. Команда работает системно и прозрачно — каждую неделю отчёты с реальными цифрами." },
  { name: "Мария С.", role: "Стартап, Болгария", text: "Начали с аудита за 19.99€ — и уже на этом этапе увидели, где теряли деньги. Сейчас на полном тарифе, результаты превзошли ожидания." },
  { name: "Дмитрий Л.", role: "SaaS, Украина", text: "Работаем с LedoLab уже полгода. Стоимость привлечения клиента снизилась на 40%, а качество лидов выросло. Рекомендую всем." },
  { name: "Анна В.", role: "Услуги, Польша", text: "LedoLab полностью перестроили наш маркетинг. Настроили воронки, CRM и аналитику. Теперь мы понимаем каждый потраченный евро." },
  { name: "Игорь П.", role: "Ритейл, Чехия", text: "Впервые за 3 года мы видим окупаемость маркетинга. Прозрачные KPI, честные отчёты, и команда реально горит результатом." },
  { name: "Елена Т.", role: "Образование, СНГ", text: "Профессиональный подход на каждом этапе. Денис лично контролирует процессы. За 2 месяца вышли на стабильный поток заявок." },
];
          ...
          {testimonials.map((item, i) => (
            <div key={i} className="h-full">
              <div className="h-full p-6 rounded-[20px] border border-border bg-surface flex flex-col">
                ...
                <p className="text-[14px] text-ink-light leading-relaxed flex-1">{item.text}</p>
                <div className="mt-4 pt-4 border-t border-border">
                  <div className="flex items-center gap-1 mb-1.5">
                    {Array.from({ length: 5 }).map((_, j) => (<Star key={j} size={13} className="text-accent-warm fill-accent-warm" />))}
                  </div>
                  <p className="font-heading text-[14px] font-semibold text-ink">{item.name}</p>
                  <p className="text-[12px] text-ink-faint">{item.role}</p>
                </div>
              </div>
            </div>
          ))}
